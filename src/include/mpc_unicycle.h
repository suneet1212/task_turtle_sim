...
class FG_eval
{
    public:
        ...
        void operator()(ADvector& fg, const ADvector X)
        {
            ...
            assert(fg.size() == (2*N)+15);
            assert(X.size() == 2*N+10);
            ...
            // retrieving constants from the input vector
            AD<double> T = X[2*N];
            ...
            std::cout << "obs_x = " << obs_x << "\n";
            std::cout << "obs_y = " << obs_y << "\n";
            ...
            // constraints for the positions to be in range.
            fg[2*N+2] = x_n;
            fg[2*N+3] = y_n;
            fg[2*N+4] = theta_n;

            // Constrainta for the constants to be constants
            fg[2*N+5] = T;
            fg[2*N+6] = target_x;
            fg[2*N+7] = target_y;

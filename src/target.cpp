...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle n;
    int done;
    ros::param::get("/unicycle/done",done); // to check whether the launch file is still running
    int target_achieved;
    ros::param::get("/target_achieved",target_achieved); // to check whether the current path is completed or not 
    while ((ros::ok()) && (done == 0))
    {
        while ((target_achieved == 0) && (ros::ok()) && (done == 0))
        {
        ...
        if(done == 0) 
        {
            ...
            else if(input == 'y')
            {
                ...
                ros::param::set("/unicycle/target_x",x);
                ros::param::set("/unicycle/target_y",y);
                ros::param::set("/unicycle/target_theta",theta);
